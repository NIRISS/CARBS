.. CARBS documentation master file, created by
   sphinx-quickstart on Sun Feb 23 16:28:58 2020.
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.

CARBS: joint spectrophotometric fitting with dense_basis and grizli
===================================================================



.. toctree::
   :maxdepth: 2
   :caption: Contents:

   tutorials/setting_up.ipynb
   tutorials/CARBS_spectrophotometric_fitting_example.ipynb

Contribute
----------

Issue Tracker: https://github.com/kartheikiyer/dense_basis/issues
Source Code: https://github.com/kartheikiyer/dense_basis
Support
If you are having issues, please let me know at: kartheik.iyer@dunlap.utoronto.ca

License & Attribution
---------------------

Copyright 2020 CANUCS team. 

dense_basis is being developed by Kartheik Iyer in a public GitHub repository. The source code is made available under the terms of the MIT license.

grizli is being developed by Gabe Brammer in a public GitHub repository. The source code is made available under the terms of the MIT license.

If you make use of this code, please cite [include relevant citation].

Indices and tables
==================

* :ref:`genindex`
* :ref:`modindex`
* :ref:`search`
